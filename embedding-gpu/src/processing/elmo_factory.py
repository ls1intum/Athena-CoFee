...
from .cloud.cloud_connection import CloudConnection
...
class ELMoFactory:
    ...
    __RESOURCE_PATH = (Path.cwd() / "src/processing/resources/models").resolve()
    ...
    def get_model_for_course(self, course_id=None):
        ...
        if model_name not in ELMoFactory.ELMo_models_cache:
            ...
            try:
                ELMoFactory.ELMo_models_cache[model_name] = ElmoEmbedder(ELMoFactory.__OPTIONS_PATH, weights_path, 0)
            except FileNotFoundError:
                self.__logger.error("Model not found, Using default elmo model")
                return ElmoEmbedder(ELMoFactory.__OPTIONS_PATH, self.__DEFAULT_WEIGHT_FILE, 0)
